...
if (!empty($_POST['user_list_type']) && !empty($_POST['event'])){
    ...
    switch ($_POST['event']) {
	    ...
	    case 'diagnostic_page':
            ...
            break;
        case 'set_variable':
            $event->sendSetVariable(@$_POST['msg']);
            break;
        case 'set_volume':
            $event->sendSetVolume(@$_POST['channel']);
            break;
	case 'show_menu':
            $event->sendShowMenu();
            break;
...
function check_event(){
    ...
    if((event_obj.options[event_obj.selectedIndex].value == 'play_channel')||(event_obj.options[event_obj.selectedIndex].value == 'set_volume')){
        document.getElementById('text_channel').style.display = "";
    }else{
        document.getElementById('text_channel').style.display = "none";
    }

    if (event_obj.options[event_obj.selectedIndex].value == 'set_variable'){
        document.getElementById('msg_row').style.display = "";
     }   
...
<tr>
    ...
    <td>        
        <select name="event" id="event" onchange="check_event()">
            ...
            <option value="diagnostic_page">diagnostic_page
            <option value="set_variable">set_variable
            <option value="set_volume">set_volume
            <option value="show_menu">show_menu
        </select>
