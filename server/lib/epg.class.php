...
class Epg implements \Stalker\Lib\StbApi\Epg
{
    ...
    public function updateEpgBySetting($setting, $force = false){
        ...
        foreach ($xml->programme as $programme){
            ...
            if ($itv_id_arr){
                ...
                // var_dump($category); break;

                if(in_array('Фильм', $category) or in_array('Для взрослых', $category)) {
                    $descr = strval($this->getElementByLangCode($programme->desc, $setting['lang_code']));
                }
                ...
                if (!empty($programme->country)) {
                    $country = strval($this->getElementByLangCode($programme->country, $setting['lang_code']));
                }

                if (!empty($programme->year)) {
                    $year = strval($this->getElementByLangCode($programme->year, $setting['lang_code']));
                }

                if (isset($programme->icon)) {
                    $icon = strval($programme->icon->attributes()->src);
                    // echo $icon . "\n";
                    // break;
                }

                if (!empty($programme->rating)){
                    $rating = intval($programme->rating->value);
                }
    ...
    public function getSimpleDataTable(){
        ...
        $stb = Stb::getInstance();
        $stb_type = $stb->getParam('stb_type');
