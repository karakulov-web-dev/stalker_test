            ...
            case 2: // obtained information about audio and video tracks of media content
             ...
             var time = stb.GetMediaLen();
              ...
              if (!empty(array_of_objects)) {
                 first_sub_lang = array_of_objects[0].lang[0]; } //Обычно русские сабы располагаются в первой дороге. 
   ...
   //alert(request.itemsPerRequest);
     ...
     serial_status = data.serial_status_text;
             ...
             var time = duration;
              ...
              if (catalog==1) {
               if (request.itemsPerRequest==12) { //для разрешения 1280
//                   byID('modalcaption_descr').style.height = '300px';   
  //                 byID('modalcaption_text').style.fontSize = '24px';   
                } else {
    //               byID('modalcaption_descr').style.height = '200px';   
      //             byID('modalcaption_text').style.fontSize = '17px';   
                } 
               genre = genre?'<br><label style="color:#28b6fc">Жанр: </label>'+genre:'';
               title = title?'<br><label style="color:#28b6fc">Название: </label>'+title:'';
               document.getElementById('modalcaption_text').innerHTML = title+genre;
               ...
               if (request.itemsPerRequest==12) { //для разрешения 1280
        //           byID('modalcaption_descr').style.height = '180px';   
          //         byID('modalcaption_text').style.fontSize = '22px';   
                } else {
            //       byID('modalcaption_descr').style.height = '145px';   
              //     byID('modalcaption_text').style.fontSize = '15px';   
                } 
               time_obj.hours = Math.floor(time / 3600);   // current move duration in hours
               time_obj.minuts = Math.floor((time - time_obj.hours * 3600) / 60);   // current move duration in minuts
               time_obj.seconds = time - time_obj.hours * 3600 - time_obj.minuts * 60;   // current move duration in seconds
               ...
               document.getElementById('modalcaption_text').innerHTML = title+year+genre+dur_txt+kp_rating_txt+imdb_rating_txt+agelimit+interpretation+serial_status+actors+produser;
               document.getElementById('modalcaption_descr').innerHTML = description; 
               var scrollHeight = document.getElementById('modalcaption_descr').scrollHeight;
               var clientHeight = document.getElementById('modalcaption_descr').clientHeight;
               byID('modalcaption_arrow').style.display = (scrollHeight>clientHeight) ? 'block' : 'none'; 
    ...
    */
    ...
    "options":{
        ...
        "subMenuItems":[["single","repeat","list","random"],["desc","asc"],["name","year","add","views","RatingKP","RatingIMDB"],["sub_on","sub_off"],["vol_95","vol_80","vol_60","vol_40","vol_20"],["skin_0","skin_1"]],
        "currentLevel":-1,
        "menuItem":0,
        "subMenuItem":0,
        "show":function(){
