  ...
  if ((stb.RDir("getenv ad")!=0)||(stb.RDir("getenv ad")==''))
     {   
      ...
      if (graphicres_mode==720)
        {
              var url = 'http://xn--d1abdw2b.net/include/kliktv_multloading_inc.php';
        }
        else
        {
            var url = 'http://xn--d1abdw2b.net/include/kliktv_multloadinghd_inc.php';
        }
      xmlhttp.onreadystatechange = function() {
        ...
        if (xmlhttp.responseText.indexOf("img")>-1)
       {
         ...
         document.getElementById('loader').innerHTML=xmlhttp.responseText+'<div id="loader1"><img align="middle" src="img/load.gif"> &nbsp;&nbsp;Загрузка</div>';
         setTimeout('document.getElementById("loader").style.display="none";',5000);   
        } else {
         clearTimeout(timer_videocatalog);
         document.getElementById('loader').innerHTML='<img src="img/main' + win.height + '.jpg"/><div id="loader1"><img align="middle" src="img/load.gif"> &nbsp;&nbsp;Загрузка</div>';
         setTimeout('document.getElementById("loader").style.display="none";',5000);           
        }
       }
     }
...
function fillPage() {
    ...
    //alert("current.cat.url="+current.priority);
...
function rebuildCurrentGlobalObj(list) {
    ...
    Deferred.next(function(){
        ...
        for(var i=0; i < list.data.items.length + tmp.length; i++) {
            if(i < tmp.length) {
                ...
                images[counter - 1].onload = function(){counter_load++;}  // event load image
                //alert [tmp_list[7]];
            }
        }
        while(counter_load==counter_wait) { // cycle for a waiting loading images
 ...
 //      alert ("klk");
        ...
        //alert ('current.id_youtube='+current.globalObj[1].id_youtube);
        //alert ('current.globalObj_rebuildCurrentGlobalObj='+current.globalObj[1].img);
        //alert ('current.globalObj_rebuildCurrentGlobalObj='+current.globalObj[1].server);
        
        //current.globalObj[1].server;
