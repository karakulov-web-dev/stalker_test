    ...
    try {
        ...
        request.onreadystatechange = function ()
        {
            if (request.readyState == 4 && request.status == 200) {
                //log("Url " + url_request + " get done");
                //alert ("Url " + url_request + " get done");
                //alert('request.responseText='+request.responseText);
                //if (url_request!=='http://clip.mezhdu.net/stb_mag200_request.php'){
                //alert ('parseYoutubePage');
                //parseYoutubePage(request.responseText);}     // call to function parse YouTube html
                //else {
                //alert ('parseClipPage');
                parseClipPage(url);     // call to function parse YouTube html
                //if (server=='youtube') {
                //alert ('getHtmlByUrl_server='+server);
                setTimeout(function(){ stb.EnableSetCookieFrom(".video.rikt.ru", true); }, 500);
                //}// for rev. 1.17
                //else setTimeout(function(){ stb.EnableSetCookieFrom(".clip.mezhdu.net", true); }, 500);// for rev. 1.17
                //setTimeout(function(){ set_coookie('.youtube.com'); }, 500);// for rev. 1.16
