...
function common_xpcom(){
    ...
    //
    this.ip  = '128.128.128.128';
    this.mac = '00:1A:79:01:83:74';
    this.type  = 'MAG254';
    // this.access_token = '60033.768419d448a8c1811abf0e50ec615445';
    //
    ...
    this.get_stb_params = function (){
	    ...
	    _debug('!!!!!!!!!!this.header_ua_ext.push: ',  stb.header_ua_ext.join('; '));
            ...
            if (stb.GetWifiLinkStatus){
                var link = [];
		          var stream_type = 0;
                stream_type = stb.RDir('getenv stream_type');
		_debug('stream_type: ', stream_type);  //
		...
		{									//
			if (stream_type==1) link.push('Ethernet');   //
			if (stream_type==2) link.push('WiFi');	//
		}									//
    ...
    this.user_init = function(user_data){
        ...
        if (1 == 2 && this.user['allowed_stb_types'] && this.user['allowed_stb_types'].indexOf(cut_type.toLowerCase()) == -1 && !_GET['debug_key']){
...
//            this.timer_id = window.setInterval(function(){self.load()}, (stb.type == 'MAG200' ? 3 : 6)*3600000);
		this.timer_id = window.setInterval(function(){self.load()}, 6*3600000);
        },
...
var connection_problem = {
    ...
    show : function(){
        ...
        if ((stb.user.enable_connection_problem_indication == 1)&&(stb.mac.substring(0,8)!="10:27:BE")){
            this.dom_obj.show();
            this.on = true;
        }
    },
