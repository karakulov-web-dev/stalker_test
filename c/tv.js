...
(function() {
  ...
  function tv_constructor() {
    ...
    if (stb.graphic_mode == 1080) {
      this.preview_pos = this.preview_pos_map[
        this.preview_pos_map.getIdxByVal("mode", parseInt(stb.video_mode))
      ];
    ...
    this.password_input = new password_input({
      parent: this,
      proceed_navigation: true
    });
    ...
    this.init = function() {
      ...
      this.parent_password_promt.addItem(
        new ModalFormInput({
      ...
      this.parent_password_promt.addItem(
        new ModalFormButton({
          ...
          onclick: function() {
            var parent_password = scope.parent_password_promt
              .getItemByName("parent_password")
              .getValue();
            ...
            if (parent_password == stb.user.parent_password) {
              ...
              scope.parent_password_promt.callback &&
                scope.parent_password_promt.callback();
            } else {
              scope.parent_password_promt.setStatus(
                get_word("parent_password_error")
              );
            }
          }
        })
    ...
    this.show = function(do_not_load) {
      ...
      if (
        this.data_items &&
        this.data_items[this.cur_row] &&
        this.data_items[this.cur_row].id &&
        stb.player.cur_media_item &&
        this.data_items[this.cur_row].id == stb.player.cur_media_item.id
      ) {
        do_not_load = true;
      } else {
        do_not_load = false;
      ...
      try {
        ...
        if (this.cur_view == "short") {
          stb.SetTopWin(1);
          stb.SetViewport(
            this.preview_pos.xsize,
            this.preview_pos.ysize,
            this.preview_pos.x,
            this.preview_pos.y
          );
    ...
    this.bind = function() {
      ...
      (function() {
        if (this.fav_manage_mode) {
          ...
          if (
            stb.profile["play_in_preview_only_by_ok"] &&
            this.cur_view == "short"
          ) {
            if (
              this.data_items[this.cur_row].id !=
      ...
      (function() {
        if (module.epg_simple) {
          ...
          if (
            this.data_items[this.cur_row].lock == 1 &&
            !this.data_items[this.cur_row].unlocked &&
            this.genre.alias !== "for adults"
          ) {
            return;
          }
    ...
    this.check_for_play = function() {
      ...
      if (!this.data_items[this.cur_row].open) {
        if (
          this.data_items[this.cur_row].hasOwnProperty("error") &&
          this.data_items[this.cur_row].error
        ) {
          stb.notice.show(
            get_word("error_channel_" + this.data_items[this.cur_row].error)
          );
        } else {
      ...
      if (
        this.data_items[this.cur_row].lock &&
        !this.data_items[this.cur_row].unlocked &&
        this.genre.alias != "for adults"
      ) {
        var self = this;
    ...
    this.fill_list = function(data) {
      ...
      if (this.fav_manage_mode && !empty(this.cur_row_data)) {
        ...
        var next_row_fav_idx = stb.player.fav_channels_ids.indexOf(
          next_item_id
        );
    ...
    this.sync_channels = function(data) {
      ...
      data = data.map(function(channel) {
        ...
        if (idx !== null) {
          ...
          var cmd_idx = channel.cmds.getIdxByVal(
            "url",
            stb.player.channels[idx].cmd
          );
          ...
          if (!stb.player.channels[idx].manually_selected || cmd_idx === null) {
            ...
            stb.player.channels[idx].use_http_tmp_link =
              channel.use_http_tmp_link;
            stb.player.channels[idx].wowza_tmp_link = channel.wowza_tmp_link;
            stb.player.channels[idx].nginx_secure_link =
              channel.nginx_secure_link;
            stb.player.channels[idx].use_load_balancing =
              channel.use_load_balancing;
        ...
        if (
          stb.profile["play_in_preview_only_by_ok"] &&
          channel.id == stb.player.cur_tv_item.id &&
          stb.player.on
        ) {
        ...
        if (
          stb.profile.ts_enable_icon &&
          module.time_shift_local &&
          module.time_shift_local.enabled &&
          channel.allow_local_timeshift == 1 &&
          channel.archive != 1
        ) {
        ...
        if (
          channel.pvr == 1 &&
          (channel.allow_local_pvr == 0 || !module.pvr_local) &&
          (channel.allow_pvr == 0 || !module.remote_pvr)
        ) {
          channel.pvr = 0;
        }
    ...
    this.fill_short_epg = function(epg) {
      ...
      for (var i = 0; i < epg.length; i++) {
        ...
        txt +=
          '<div class="mark_memo" style="display:' +
          (!!epg[i].mark_memo ? "block" : "none") +
          '"></div>';
        txt +=
          '<div class="dummy_mark_memo" style="display:' +
          (!!epg[i].mark_memo ? "none" : "block") +
          '"></div>';
    ...
    this._play_now = function(item) {
      ...
      if (
        stb.player.cur_media_item &&
        stb.player.cur_media_item.id &&
        item.id == stb.player.cur_media_item.id &&
        stb.player.on
      ) {
        return;
      }
    ...
    this.del_from_fav = function() {
      ...
      var fav_idx = stb.player.fav_channels_ids.indexOf(
        this.data_items[this.cur_row].id
      );
      ...
      var fav_ch_idx = stb.player.fav_channels.getIdxByVal(
        "id",
        this.data_items[this.cur_row].id
      );
    ...
    this.shift_row = function(dir) {
      if (this.fav_manage_mode) {
        ...
        _debug(
          "stb.player.fav_channels_ids before",
          stb.player.fav_channels_ids
        );
      }
      ...
      if (this.fav_manage_mode) {
        ...
        if (
          this.data_items[this.cur_row].hasOwnProperty("open") &&
          !this.data_items[this.cur_row].open
        ) {
        ...
        if (
          this.data_items[cur_row_num].hasOwnProperty("open") &&
          !this.data_items[cur_row_num].open
        ) {
        ...
        _debug(
          "stb.player.fav_channels_ids after",
          stb.player.fav_channels_ids
        );

        for (var j = 0; j < this.row_blocks.length; j++) {
          this.handling_block(
            cur_row_data[this.row_blocks[j]],
            this.map[this.cur_row],
            this.row_blocks[j]
          );

          this.handling_block(
            next_row_data[this.row_blocks[j]],
            this.map[cur_row_num],
            this.row_blocks[j]
          );
        }
      }
    ...
    this.init_quick_ch_switch = function() {
      ...
      this.quick_ch_switch.dom_obj = create_block_element(
        "quick_ch_switch tv_preview_quick_ch_switch"
      );
    ...
    this.quick_go_to_ch = function() {
      ...
      if (stb.user.fav_itv_on) {
        stb.player.f_ch_idx = stb.player.fav_channels.getIdxByVal(
          "number",
          ch_num
        );
      ...
      if (!empty(item)) {
        if (this.cur_view == "short") {
          ...
          stb.player.send_last_tv_id_callback = function() {
            self.load_data.apply(self);
          };
          stb.player.need_show_info = 0;
          this._play_now(item);
        } else {
      ...
      load: function() {
        ...
        if (this.parent && this.parent.data_items && this.parent.cur_row >= 0) {
          if (this.parent.data_items[this.parent.cur_row].type == "dvb") {
            this.fill(
              stb.epg_loader.get_curr_and_next(
                this.parent.data_items[this.parent.cur_row].id,
                false,
                5
              )
            );
    ...
    this.progress_bar = {
      ...
      init: function(parent) {
        ...
        this.dom_obj = create_block_element(
  ...
  if (stb.profile["tv_quality_filter"]) {
    color_buttons_map.push({
      label: word["tv_favorite"],
      cmd: tv.fav_menu_switcher
    });
    color_buttons_map.push({
      label: get_word("tv_quality"),
      cmd: tv.filter_switcher
    });
  } else {
  ...
  var sort_menu_map = [
    {
      ...
      cmd: function() {
        ...
        if (!stb.profile["tv_quality_filter"]) {
          ...
          this.parent.fav_menu &&
            this.parent.fav_menu.disable_by_name("fav_manage");
        }
  ...
  if (stb.profile["tv_quality_filter"]) {
    ...
    tv.init_filter_menu(filter_menu_map, {
      color: "blue",
      need_update_header: false
    });
  }

  tv.init_sort_menu(sort_menu_map, {
    offset_x: 217,
    color: "green"
  });
  ...
  tv.init_view_menu(
    [
      {
        label: word["tv_list_w_info"],
        cmd: function() {
          this.parent.set_short_container();
        }
      },
      {
        label: word["tv_list"],
