    ...
    stbEvent.onPortalEvent = function(params) {
      ...
      if (params.hasOwnProperty("type") && params.type == "settings") {
        ...
        if (params.hasOwnProperty("plasma_saving")) {
          stb.user.plasma_saving = stb.profile.plasma_saving =
            params.plasma_saving;
        }

        if (params.hasOwnProperty("ts_enable_icon")) {
          stb.user.ts_enable_icon = stb.profile.ts_enable_icon =
            params.ts_enable_icon;
        }
        ...
        if (params.hasOwnProperty("ts_max_length")) {
          stb.user.ts_max_length = stb.profile.ts_max_length =
            params.ts_max_length;
        }

        if (params.hasOwnProperty("ts_buffer_use")) {
          stb.user.ts_buffer_use = stb.profile.ts_buffer_use =
            params.ts_buffer_use;
        }

        if (params.hasOwnProperty("ts_action_on_exit")) {
          stb.user.ts_action_on_exit = stb.profile.ts_action_on_exit =
            params.ts_action_on_exit;
        }
        ...
        if (params.hasOwnProperty("hdmi_event_reaction")) {
          stb.user.hdmi_event_reaction = stb.profile.hdmi_event_reaction =
            params.hdmi_event_reaction;
        }

        if (params.hasOwnProperty("pri_audio_lang")) {
          stb.user.pri_audio_lang = stb.profile.pri_audio_lang =
            params.pri_audio_lang;
        }

        if (params.hasOwnProperty("sec_audio_lang")) {
          stb.user.sec_audio_lang = stb.profile.sec_audio_lang =
            params.sec_audio_lang;
        }

        if (
          params.hasOwnProperty("pri_audio_lang") &&
          params.hasOwnProperty("pri_audio_lang")
        ) {
          stb.player.set_audio_langs(
            stb.user.pri_audio_lang,
            stb.user.sec_audio_lang
          );
        }

        if (params.hasOwnProperty("pri_subtitle_lang")) {
          stb.user.pri_subtitle_lang = stb.profile.pri_subtitle_lang =
            params.pri_subtitle_lang;
        }

        if (params.hasOwnProperty("sec_subtitle_lang")) {
          stb.user.sec_subtitle_lang = stb.profile.sec_subtitle_lang =
            params.sec_subtitle_lang;
        }

        if (
          params.hasOwnProperty("pri_subtitle_lang") &&
          params.hasOwnProperty("sec_subtitle_lang")
        ) {
          stb.player.set_subtitle_langs(
            stb.user.pri_subtitle_lang,
            stb.user.sec_subtitle_lang
          );
        }

        if (params.hasOwnProperty("play_in_preview_by_ok")) {
          stb.user.play_in_preview_only_by_ok = stb.profile.play_in_preview_only_by_ok =
            params.play_in_preview_by_ok;
        }
      ...
      if (msg == "show") {
        if (data) {
          ...
          if (data.module && module[data.module]) {
            ...
            module[stb.cur_layer.layer_name].hide &&
              module[stb.cur_layer.layer_name].hide();
      ...
      for (var param in profile["cas_additional_params"]) {
        if (profile["cas_additional_params"].hasOwnProperty(param)) {
          _debug(
            ...
            param,
            profile["cas_additional_params"][param]
          );
          stb.SetAdditionalCasParam(
            param,
            profile["cas_additional_params"][param]
...
player.prototype.init_time_shift_exit_confirm = function() {
  ...
  this.time_shift_exit_confirm = new ModalForm({
    title: get_word("confirm_form_title"),
    text: get_word("time_shift_exit_confirm_text")
  });
  ...
  this.time_shift_exit_confirm.addItem(
    new ModalFormButton({
      ...
      onclick: function() {
        ...
        if (scope.active_local_time_shift) {
          ...
          if (scope.cur_media_item.timeshift_hist_id) {
            scope.update_played_timeshift_end_time(
              scope.cur_media_item.timeshift_hist_id
            );
          }
...
player.prototype.init_pvr_dialogs = function() {
  ...
  this.pvr_target_select = new ModalForm({
    title: get_word("select_form_title"),
    text: get_word("pvr_target_select_text")
  });
  this.pvr_target_select.getTextDomObj().style.textAlign = "center";
  this.pvr_target_select.enableOnExitClose();

  this.pvr_target_select.addItem(
    new ModalFormButton({
      ...
      onclick: function() {
        ...
        if (scope.pvr_target_select.deferred) {
          scope.local_pvr_confirm
            .getItemByName("start_btn")
            .setValue(get_word("add_btn"));
        } else {
          scope.local_pvr_confirm
            .getItemByName("start_btn")
            .setValue(get_word("start_btn"));
        }
        scope.pvr_target_select.hide();
        scope.local_pvr_confirm.show({
          parent: scope.pvr_target_select._parent
        });
      }
    })
  );
  ...
  this.pvr_target_select.addItem(
    new ModalFormButton({
      ...
      onclick: function() {
        ...
        if (scope.pvr_target_select.deferred) {
          scope.remote_pvr_confirm.show({
            text: get_word("remote_deferred_pvr_confirm_text"),
            parent: scope.pvr_target_select._parent
          });
        } else {
          scope.remote_pvr_confirm.show({
            text: get_word("remote_pvr_confirm_text"),
            parent: scope.pvr_target_select._parent
          });
        }
      }
    ...
    new ModalFormButton({
      ...
      onclick: function() {
        if (scope.remote_pvr_confirm.deferred) {
          scope.remote_pvr_confirm._parent.recorder.add_remote(
            scope.remote_pvr_confirm.program
          );
        } else {
  ...
  this.local_pvr_confirm = new ModalForm({
    title: get_word("rec_options_form_title"),
    id: "local_pvr_confirm"
  });
  ...
  this.local_pvr_confirm.addItem(
    new ModalFormDateTimeSelect({
      ...
      onset: function(value) {
        ...
        var start_time = scope.local_pvr_confirm
          .getItemByName("start_time")
          .getDateValue();
        var end_time = scope.local_pvr_confirm
          .getItemByName("end_time")
          .getDateValue();
  ...
  this.local_pvr_confirm.addItem(
    new ModalFormDateTimeSelect({
      ...
      onset: function(value) {
        ...
        var duration =
          parseInt(duration_parts[0], 10) * 3600 +
          parseInt(duration_parts[1], 10) * 60;
  ...
  this.local_pvr_confirm.addItem(
    new ModalFormSelect({
      label: get_word("usb_device") + ":",
      name: "usb_device"
    })
  );
  this.local_pvr_confirm.addItem(
    new ModalFormInput({
      label: get_word("file_name") + ":",
      name: "file_name"
    })
  );
  ...
  this.local_pvr_confirm.addItem(
    new ModalFormButton({
      ...
      onclick: function() {
        var path =
          scope.local_pvr_confirm.getItemByName("usb_device").getValue() +
          "/" +
          scope.local_pvr_confirm.getItemByName("file_name").getValue();
        ...
        if (
          start_time.getTime() > now_time.getTime() &&
          !scope.local_pvr_confirm.deferred
        ) {
          ...
          if (
            scope.local_pvr_confirm.program.start_timestamp * 1000 !=
              start_time.getTime() ||
            scope.local_pvr_confirm.program.stop_timestamp * 1000 !=
              end_time.getTime()
          ) {
            scope.local_pvr_confirm.program.start_timestamp = parseInt(
              start_time.getTime() / 1000,
              10
            );
            scope.local_pvr_confirm.program.stop_timestamp = parseInt(
        ...
        if (
          scope.local_pvr_confirm.deferred &&
          scope.local_pvr_confirm.program.id == 0
        ) {
          module.pvr_local.create_for_program(
            scope.local_pvr_confirm.program,
            path
          );
        } else if (scope.local_pvr_confirm.deferred) {
          scope.local_pvr_confirm._parent.recorder.add_local(
            scope.local_pvr_confirm.program,
            path
          );
        } else {
          module.pvr_local.create(
            scope.local_pvr_confirm.channel,
            path,
            null,
            parseInt(end_time.getTime() / 1000, 10)
          );
        }
  ...
  this.local_pvr_confirm.addCustomEventListener("before_show", function() {
    if (scope.local_pvr_confirm.program && scope.local_pvr_confirm.deferred) {
      ...
      var epg = stb.epg_loader.get_curr_and_next(
        scope.local_pvr_confirm.channel.id
      );
    ...
    scope.local_pvr_confirm
      .getItemByName("end_time")
      ._onset(scope.local_pvr_confirm.getItemByName("end_time").getValue());
  });
  ...
  this.local_pvr_confirm.addCustomEventListener("show", function() {
    ...
    if (scope.local_pvr_confirm.deferred) {
      scope.local_pvr_confirm
        .getItemByName("file_name")
        .setValue(
          module.pvr_local.get_record_filename_for_program(
            scope.local_pvr_confirm.program
          )
        );
    } else {
      scope.local_pvr_confirm
        .getItemByName("file_name")
        .setValue(
          module.pvr_local.get_record_filename(scope.local_pvr_confirm.channel)
        );
    }
  });
};
...
player.prototype.init_subtitle_encoding_select = function() {
  ...
  this.subtitle_encoding_select = new ModalForm({
    title: get_word("select_form_title")
  });
  this.subtitle_encoding_select.enableOnExitClose();

  this.subtitle_encoding_select.addItem(
    new ModalFormSelect({
  ...
  this.subtitle_encoding_select.addItem(
    new ModalFormButton({
      ...
      name: "ok_btn",
      onclick: function() {
        var subtitle_encoding = scope.subtitle_encoding_select
          .getItemByName("subtitle_encoding")
          .getValue();

        _debug("subtitle_encoding", subtitle_encoding);

        scope.subtitle_encoding_select.on_confirm &&
          scope.subtitle_encoding_select.on_confirm(subtitle_encoding);
...
player.prototype.init_play_or_download_dialog = function() {
  ...
  this.play_or_download_dialog = new ModalForm({
    title: get_word("play_or_download")
  });
  this.play_or_download_dialog.enableOnExitClose();

  this.play_or_download_dialog.addItem(
    new ModalFormButton({
  ...
  this.play_or_download_dialog.addItem(
    new ModalFormButton({
      ...
      onclick: function() {
        module.downloads.dialog.show({
          parent: main_menu,
          url: stb.player.play_or_download_dialog.url
        });
  ...
  switch (event) {
    case 1: {
      ...
      try {
        ...
        if (
          this.media_type == "stream" &&
          (this.is_tv || stb.cur_place == "radio")
        ) {
          _debug("stream error");
          ...
          _debug(
            "stb.user.tv_playback_retry_limit",
            stb.user.tv_playback_retry_limit
          );
          _debug(
          ...
          if (
            stb.user.tv_playback_retry_limit > 0 &&
            this.event1_counter > stb.user.tv_playback_retry_limit
          ) {
            _debug("stop replay");
            return;
          }
          ...
          if (this.is_tv) {
            ...
            this.replay_channel_timer = window.setTimeout(function() {
              self.play_last();
            }, replay_timeout);
          } else {
            this.replay_channel_timer = window.setTimeout(function() {
          ...
          if (
            this.cur_media_item.media_type != "vclub_ad" &&
            this.play_continuously &&
            this.cur_media_item.hasOwnProperty("series") &&
            this.cur_media_item.series &&
            this.cur_media_item.series.length > 0
          ) {
            _debug(
              "this.cur_media_item.cur_series before",
              this.cur_media_item.cur_series
            );
            _debug(
              "this.cur_media_item.series.length",
              this.cur_media_item.series.length
            );
            _debug(
              "this.cur_media_item.hasOwnProperty(series)",
            ...
            var series_idx = this.cur_media_item.series.indexOf(
              this.cur_media_item.cur_series
            );
            ...
            if (old_series_idx != series_idx) {
              this.cur_media_item.cur_series = this.cur_media_item.series[
                series_idx
              ];
              ...
              if (this.cur_media_item.cmd.indexOf("://")) {
                this.cur_media_item.cmd =
                  "/media/" + this.cur_media_item.id + ".mpg";
              }
            ...
            this.cur_media_item.playlist &&
            this.cur_media_item.playlist.length > 0
          ) {
            stb.key_lock = false;
            ...
            if (typeof this.cur_media_item.playlist[0] == "object") {
              ...
              for (var i = 0; i < this.cur_media_item.playlist.length; i++) {
                if (
                  this.cur_media_item.id == this.cur_media_item.playlist[i].id
                ) {
                  idx = i;
                  break;
                }
              ...
              var idx = this.cur_media_item.playlist.indexOf(
                this.cur_media_item.cmd
              );
              ...
              if (idx >= 0 && idx < this.cur_media_item.playlist.length - 1) {
                ...
                var osd_title_match = /osd_title=([^&]*)/.exec(
                  cur_media_item.cmd
                );

                if (osd_title_match) {
                  cur_media_item.name = decodeURIComponent(
                    osd_title_match[1].replace(/\+/g, "%20")
                  );
                } else if (
                  !cur_media_item.hasOwnProperty("keep_original_name") ||
                  !cur_media_item.keep_original_name
                ) {
                  cur_media_item.name = cur_media_item.cmd.substr(
                    this.cur_media_item.cmd.lastIndexOf("/") + 1
                  );
                }
                ...
                if (
                  this.cur_media_item.media_type &&
                  this.cur_media_item.media_type == "vclub_ad"
                ) {
                  ...
                  module &&
                    module.vclub &&
                    module.vclub.set_ad_ended_time &&
                    module.vclub.set_ad_ended_time(
                      this.cur_media_item.ad_id,
                      stb.GetPosTime(),
                      stb.GetMediaLen(),
                      true
                    );
    ...
    case 2: {
      // Receive information about stream
      ...
      if (stb.GetMetadataInfo) {
        ...
        if (
          metadata &&
          metadata.hasOwnProperty("titles") &&
          ...
          this.cur_media_item.playlist = metadata.titles.map(function(
            title,
            idx
          ) {
          ...
          if (metadata.hasOwnProperty("infoCurtitle")) {
            this.cur_media_item.cmd =
              clear_cmd + "?title=" + metadata.infoCurtitle;
          }
        }
      ...
      //if (this.is_tv){
      if (
        stb.user["enable_buffering_indication"] &&
        this.cur_media_item.cmd.indexOf("rtp ") != 0
      ) {
    ...
    case 4: {
      ...
      if (this.cur_media_item.hasOwnProperty("volume_correction")) {
        this.volume.correct_level(
          parseInt(this.cur_media_item.volume_correction)
        );
      } else {
        this.volume.correct_level(0);
      }
      ...
      if (
        this.is_tv &&
        this.cur_tv_item &&
        this.cur_tv_item.ready_to_timeshift &&
        module.time_shift_local &&
        module.time_shift_local.enabled
      ) {
      ...
      if (this.active_local_time_shift && module.time_shift_local) {
        ...
        if (
          this.cur_media_item.hasOwnProperty("ad_must_watch") &&
          this.cur_media_item.ad_must_watch != "all"
        ) {
          var lock_time =
            (this.cur_media_length * this.cur_media_item.ad_must_watch) / 100;
      ...
      if (module.tv_archive && this.cur_media_item.mark_archive) {
        ...
        var archive_continue_dialog_delay =
          (this.cur_media_length - this.cur_pos_time - 30) * 1000;

        _debug(
          "archive_continue_dialog_delay 1",
    ...
    case 5: {
      ...
      if (this.media_type == "stream") {
        ...
        if (
          stb.user.tv_playback_retry_limit > 0 &&
          this.event5_counter > stb.user.tv_playback_retry_limit
        ) {
          _debug("stop replay");
          return;
        }
    ...
    case 33: {
      ...
      _debug(
        "stb.profile[hdmi_event_reaction]",
        stb.profile["hdmi_event_reaction"]
      );
      ...
      if (stb.profile["hdmi_event_reaction"]) {
        ...
        _debug(
          "stb.GetHDMIConnectionState()",
          typeof stb.GetHDMIConnectionState == "function"
            ? stb.GetHDMIConnectionState()
            : undefined
        );

        // wait 3s to get more real GetHDMIConnectionState
        var hdmi_reaction_timeout =
          (stb.profile["hdmi_event_reaction"] == 1
            ? 3
            : stb.profile["hdmi_event_reaction"]) *
          1000 *
          (typeof stb.GetHDMIConnectionState == "function" &&
          stb.GetHDMIConnectionState() == 2
            ? 0
            : 1);
    ...
    case 36: {
      ...
      if (
        params.event_code === 3 &&
        this.pause.on &&
        stb.profile.ts_buffer_use === "cyclic"
      ) {
        this.disable_pause();
    ...
    case 40: {
      // Scanning DVB Channel in progress (STB_EVENT_DVB_SCAN_PROGRESS)
      _debug("params", params);
      module.dvb && module.dvb.on_scan_result("progress", params);
      break;
    }
    case 41: {
      // Scanning DVB Channel found (STB_EVENT_DVB_SCAN_FOUND)
      _debug("params", params);
      module.dvb && module.dvb.on_scan_result("found", params);
      break;
    }
    case 42: {
      // DVB EPG update (STB_EVENT_DVB_EPG_UPDATE)
      _debug("params", params);
...
player.prototype.volume = new (function() {
  ...
  this._get_real_volume_level = function(level) {
    if (
      stb.profile["logarithm_volume_control"] &&
  ...
  if (cmd.indexOf("://") > 0) {
    ...
    if (
      (cmd.indexOf("mmsh://") >= 0 ||
        ...
        cmd.indexOf("http://") >= 0 ||
        cmd.indexOf("dvb://") >= 0) &&
      !this.active_time_shift &&
      stb.cur_place != "demo" &&
      stb.cur_place != "internet" &&
      stb.cur_place != "epg_simple" &&
      stb.cur_place != "epg" &&
      stb.cur_place != "radio" &&
      stb.cur_place != "vclub" &&
      stb.cur_place != "karaoke" &&
      stb.cur_place != "audioclub" &&
      !this.cur_media_item.is_audio &&
      !this.cur_media_item.promo &&
      !this.cur_media_item.radio
    ) {
      this.is_tv = true;
    } else {
      this.is_tv = false;
...
player.prototype.init_first_channel = function() {
  ...
  if (
    typeof this.channels != "undefined" &&
...
player.prototype.play = function(item) {
  ...
  if (
    item.hasOwnProperty &&
    item.hasOwnProperty("media_len") &&
    parseInt(item.media_len) > 0
  ) {
  ...
  if (stb.usbdisk.storage_info.length > 0) {
    cmd = cmd.replace(
      /\/media\/USB-\*/,
      stb.usbdisk.storage_info[0]["mountPath"]
    );
  }
  ...
  if (
    module.tv_archive &&
    this.cur_media_item.mark_archive &&
    this.cur_media_item.archive_hist_id
  ) {
  ...
  if (this.active_time_shift && this.cur_media_item.timeshift_hist_id) {
    this.update_played_timeshift_end_time(
      this.cur_media_item.timeshift_hist_id
    );
  ...
  if (this.file_type == "audio" || this.cur_media_item.is_audio) {
    ...
    stb.player.cur_media_item.radio ||
    typeof stb.player.radio_idx != "undefined"
  ) {
  ...
  if (
    this.media_type == "stream" &&
    !stb.player.cur_media_item.hasOwnProperty("series")
  ) {
    if (item.hasOwnProperty("open") && !item.open) {
      ...
      if (
        !this.active_time_shift &&
        !this.active_local_time_shift &&
        (parseInt(item.use_http_tmp_link) == 1 ||
          parseInt(item.use_load_balancing) == 1)
      ) {
        var self = this;
        stb.player.on_create_link = function(result) {
          ...
          if (result.error) {
            ...
            module.tv.preview_msg.innerHTML = get_word(
              "error_channel_" + result.error
            );
          } else {
    ...
    cmd.indexOf("/usbdisk") > 0 ||
    cmd.indexOf("/USB-") > 0 ||
    cmd.indexOf("/ram/mnt/smb/") > 0 ||
    cmd.indexOf("/av/") > 0 ||
    cmd.indexOf("/UPnP/") > 0 ||
    stb.cur_place == "media_browser"
  ) {
    this.play_now(cmd);
  } else if (stb.cur_place == "karaoke") {
    this.create_link("karaoke", cmd, 0);
  } else if (
    stb.cur_place == "records" ||
    stb.cur_place == "remote_pvr" ||
    stb.cur_place == "epg_simple" ||
    stb.cur_place == "epg"
  ) {
...
player.prototype.create_link = function(
  type,
  uri,
  series_number,
  forced_storage,
...
player.prototype.play_now = function(item) {
  ...
  if (
    module.time_shift_local &&
    module.time_shift_local.enabled &&
    this.cur_media_item.allow_local_timeshift === "1" &&
    (uri.indexOf("rtp ") === 0 || uri.indexOf("ffrt ") === 0)
  ) {
    ...
    if (this.cur_media_item.error) {
      ...
      if (use_proxy) {
        ...
        if (stb.user["web_proxy_user"]) {
          proxy_addr +=
            stb.user["web_proxy_user"] + ":" + stb.user["web_proxy_pass"] + "@";
        }
        proxy_addr +=
          stb.user["web_proxy_host"] + ":" + stb.user["web_proxy_port"];
...
player.prototype.stop = function() {
  ...
  if (this.media_type == "file") {
    ...
    this.umount_timer = window.setTimeout(function() {
      stb.Umount(storage);
    }, 10000);
  }
  ...
  if (stb.cur_place == "vclub" && this.play_auto_ended) {
    if (this.cur_media_item.media_type == "vclub_ad") {
      ...
      module &&
        module.vclub &&
        module.vclub.set_ended &&
        module.vclub.set_ended(this.cur_media_item.id);
    }
  }
...
player.prototype.show_rec_icon = function(record) {
  ...
  stb.player.rec.set_seconds(
    stb.clock.convert_sec_to_human_time(
      record["t_start_ts"] - stb.clock.timestamp
    )
  );
...
player.prototype.tick_s = function(record) {
  stb.player.rec.set_seconds(
    stb.clock.convert_sec_to_human_time(
      stb.clock.timestamp - record["t_start_ts"]
    )
  );
};
...
player.prototype.pause_switch = function() {
  ...
  _debug(
    "module.time_shift_local.enabled",
    module.time_shift_local ? !!module.time_shift_local.enabled : false
  );
  _debug(
    "this.cur_media_item.allow_local_timeshift",
    this.cur_media_item.allow_local_timeshift
  );
  ...
  if (
    this.is_tv &&
    (!module.time_shift ||
      !parseInt(this.cur_media_item.enable_tv_archive, 10)) &&
    (!module.time_shift_local ||
      !module.time_shift_local.enabled ||
      !parseInt(this.cur_media_item.allow_local_timeshift, 10) ||
      (this.cur_media_item.cmd.indexOf("rtp ") == -1 &&
        this.cur_media_item.cmd.indexOf("ffrt ") == -1)) &&
  ...
  if (this.pause.on) {
    ...
    if (
      (this.file_type == "audio" || this.cur_media_item.is_audio) &&
      this.cur_media_item.playlist
    ) {
...
player.prototype.disable_pause = function() {
  ...
  if (this.active_time_shift) {
    _debug(
      "new Date() - module.time_shift.cur_media_item.live_date",
      (new Date().getTime() -
        module.time_shift.cur_media_item.live_date.getTime()) /
        1000
    );
    ...
    try {
      if (module.tv_archive && this.cur_media_item.mark_archive) {
        ...
        var archive_continue_dialog_delay =
          (cur_media_length - stb.GetPosTime() - 30) * 1000;
...
player.prototype.init_show_info = function() {
  ...
  this.info.clock = create_block_element(
    "osd_info_clock",
    this.info["dom_obj"]
  );

  this.info.time_shift_mark = create_block_element(
    "osd_info_time_shift_mark",
    this.info["dom_obj"]
  );

  this.info.title = create_block_element(
    "osd_info_title",
    this.info["dom_obj"]
  );
  ...
  this.pos_time_bar = create_block_element(
    "pos_time",
    this.info.video_container
  );

  this.info.cur_pos_time = create_block_element(
    "cur_pos_time",
    this.pos_time_bar
  );

  var pos_time_separator = create_block_element(
    "pos_time_separator",
    this.pos_time_bar
  );
  pos_time_separator.innerHTML = "/";

  this.info.total_pos_time = create_block_element(
    "total_pos_time",
    this.pos_time_bar
  );

  this.info.pos_series = create_block_element(
    "pos_series",
    this.info.video_container
  );
...
player.prototype.show_info = function(item, direct_call) {
  ...
  try {
    if (stb.cur_place == "radio") {
      ...
      if (item.hasOwnProperty("open") && !item.open) {
        if (item.hasOwnProperty("error") && item.error) {
          this.info.epg.innerHTML =
            ...
            get_word("error_channel_" + item.error) +
            "</span>";
        } else {
          this.info.epg.innerHTML =
            '<span style="color:#ffb0b0">' +
        ...
        if (
          ...
          epg &&
          epg.length > 0 &&
          epg[0].hasOwnProperty("descr") &&
          epg[0].descr &&
          this.info.epg.getAttribute("descr") !== "1"
        ) {
      ...
      _debug(
        "this.info.video_container.isHidden()",
        this.info.video_container.isHidden()
      );
    ...
    if (item.cur_series && parseInt(item.cur_series) > 0) {
      this.info.pos_series.innerHTML =
        item.cur_series + " " + get_word("player_series");
    } else if (
      (this.cur_media_item.hasOwnProperty("live_date") || this.pause.on) &&
      this.active_time_shift
    ) {
    ...
    if (
      (item.enable_tv_archive == 1 && module.time_shift) ||
      (item.allow_local_timeshift == 1 &&
        module.time_shift_local &&
        module.time_shift_local.enabled)
    ) {
  ...
  if (dir > 0) {
    if (stb.user.fav_itv_on) {
      if (module.tv.genre && module.tv.genre.id != "*") {
        this.f_ch_idx = this._find_nearest_ch_idx(dir, {
          tv_genre_id: module.tv.genre.id
        });
      ...
      if (module.tv.genre && module.tv.genre.id != "*") {
        this.ch_idx = this._find_nearest_ch_idx(dir, {
          tv_genre_id: module.tv.genre.id
        });
...
player.prototype.send_last_tv_id = function(id) {
  ...
  stb.load(
    ...
    function(result) {
      ...
      _debug(
        "typeof this.send_last_tv_id_callback",
        typeof this.send_last_tv_id_callback
      );
...
player.prototype.time_shift_indication = {
  ...
  init: function() {
    ...
    this.time_shift_info_container = create_block_element(
      "time_shift_info_container"
    );
    this.time_shift_info_txt = create_block_element(
      "time_shift_info_block",
...
player.prototype.ad_indication = {
  ...
  init: function() {
    ...
    this.ad_indication_container = create_block_element(
      "time_shift_info_container"
    );
    this.ad_indication_txt = create_block_element(
      "time_shift_info_block",
      this.ad_indication_container
    );
...
player.prototype.ad_skip_indication = {
  ...
  init: function() {
    ...
    this.ad_skip_indication_container = create_block_element(
      "ad_skip_container"
    );
    this.ad_skip_indication_txt = create_block_element(
      "ad_skip_block",
      this.ad_skip_indication_container
    );

    this.ad_skip_indication_txt.innerHTML =
      '<div class="ad_skip_txt">' + get_word("ad_skip") + "</div>";
...
player.prototype.bind = function() {
  ...
  (function() {
    if (this.info.on) {
      ...
      if (this.active_time_shift || this.active_local_time_shift) {
        ...
        this.prev_layer.layer_name == "epg" &&
        this.prev_layer.player_overlay_mode
      ) {
        this.play_last();
  ...
  (function() {
    if (this.info.on && !this.quick_ch_switch.on) {
      ...
      this.prev_layer &&
      this.prev_layer.cur_view == "short" &&
      !this.is_tv
    ) {
      if (this.active_time_shift || this.active_local_time_shift) {
        this.show_time_shift_exit_confirm();
  ...
  (function(dir) {
    _debug(
      "stb.profile[enable_arrow_keys_setpos]",
      stb.profile["enable_arrow_keys_setpos"]
    );
    _debug("dir", dir);
    ...
    .bind(key.RIGHT, this, 1)
    .bind(key.LEFT, this, -1));
  ...
  (function() {
    if (this.is_tv && !module.tv.on) {
      ...
      var allow_local_pvr =
        module.pvr_local && this.cur_tv_item["allow_local_pvr"];
  ...
  (function() {
    ...
    .bind(key.BLUE, this));
  (function() {
    if (stb.player.TempOnVideo.on == false) {
      stb.player.TempOnVideo.changeType("upRight");
      setTimeout('stb.player.TempOnVideo.changeType("Off")', 4000);
    }
    if (stb.player.ClockOnVideo.on == false) {
      stb.player.ClockOnVideo.changeType("upRight");
      setTimeout('stb.player.ClockOnVideo.changeType("Off");', 4000);
    }
...
player.prototype.change_pos_by_numbers = function(num) {
  ...
  if (this.pos_by_numbers_input.length > 6) {
    this.pos_by_numbers_input = this.pos_by_numbers_input.substr(
      this.pos_by_numbers_input.length - 6,
      6
    );
  }
...
player.prototype.pos_by_numbers_back_key_handler = function() {
  ...
  this.pos_by_numbers_input = this.pos_by_numbers_input.substr(
    0,
    this.pos_by_numbers_input.length - 1
  );
...
player.prototype.set_pos_button = function(to_time) {
  ...
  try {
    if (this.active_time_shift) {
      ...
      if (to_time > this.cur_media_length) {
        ...
        if (!module.time_shift.is_last_archive_day()) {
          this.cur_pos_time = to_time = 0;
          //_debug('++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++!date + 1');
          module.time_shift.cur_piece_date.setDate(
            module.time_shift.cur_piece_date.getDate() + 1
          );
          this.cur_media_length = module.time_shift.get_cur_media_length();
      ...
      if (program_name) {
        this.info.title.innerHTML =
          this.cur_media_item.number +
          ". " +
          this.cur_media_item.name +
          " — " +
          program_name;
    ...
    if (this.cur_media_length != 0) {
      ...
      _debug(
        "this.info.pos_bar.style.display",
        this.info.pos_bar.style.display
      );
      to_pos = Math.round(
        ((this.info.pos_bar.offsetWidth -
          (this.info.pos_button.offsetWidth + 12)) *
          to_time) /
          this.cur_media_length
      );
    }
    ...
    if (!reset) {
      if (this.active_time_shift && module.time_shift) {
        ...
        if (new_url) {
          ...
          var new_pos_time =
            this.new_pos_time +
            module.tv_archive.get_start_position() -
            module.tv_archive.get_file_piece_num() * 3600;
      ...
      if (this.active_time_shift) {
        ...
        module.time_shift.cur_piece_date = new Date(
          module.time_shift.cur_media_item.live_date
        );
      }
    }
  } catch (e) {
...
player.prototype.move_pos = function(dir) {
  ...
  if (
    this.diff_pos + this.next_step + this.cur_pos_time >
    this.cur_media_length
  ) {
...
player.prototype.update_current_time = function(cur_time, strict) {
  ...
  this.info.total_pos_time.innerHTML = this.sec_to_human_time(
    this.cur_media_length
  );
};
...
player.prototype.pos_format_str = function(number) {
  ...
  return (
    number.substr(0, 2) + ":" + number.substr(2, 2) + ":" + number.substr(4, 2)
  );
};
...
player.prototype.init_quick_ch_switch = function() {
  ...
  this.quick_ch_switch.input = create_block_element(
    "quick_ch_input",
    this.quick_ch_switch.dom_obj
  );
...
player.prototype.show_quick_ch_switch = function(num) {
  ...
  if (this.quick_ch_switch.input.innerHTML.length < 3) {
    if (this.quick_ch_switch.input.innerHTML.length == 0 && num == 0) {
      //  	if (this.quick_ch_switch.input.innerHTML.length == 0){
    } else {
...
player.prototype.del_quick_go_ch = function() {
  ...
  this.quick_ch_switch.input.innerHTML = this.quick_ch_switch.input.innerHTML.substr(
    0,
    this.quick_ch_switch.input.innerHTML.length - 1
  );
...
player.prototype.change_aspect = function() {
  ...
  this.show_aspect_info(
    get_word("aspect_" + stb.aspect_array[stb.aspect_idx].alias).toUpperCase()
  );
...
player.prototype.change_tv_channel_aspect = function() {
  ...
  _debug(
    "this.ch_aspect[this.cur_media_item.id]",
    this.ch_aspect[this.cur_media_item.id]
  );

  if (this.ch_aspect[this.cur_media_item.id] === undefined) {
    this.ch_aspect[this.cur_media_item.id] =
      stb.aspect_array[this.ch_aspect_idx].mode;
  }
  ...
  this.show_aspect_info(
    get_word(
      "aspect_" + stb.aspect_array[this.ch_aspect_idx].alias
    ).toUpperCase()
  );
...
player.prototype.init_aspect_info = function() {
  ...
  this.aspect_info_txt = create_block_element(
    "aspect_block",
    this.aspect_info_container
  );
...
player.prototype.show_aspect_info = function(text) {
  ...
  this.aspect_info_timer = window.setTimeout(function() {
    self.aspect_info_container.hide();
  }, 3000);
};
...
player.prototype.get_pids = function() {
  ...
  try {
    ...
    _debug(
      "typeof this.subtitle_pid.get_all",
      typeof this.subtitle_pid.get_all
    );
...
player.prototype.set_media_aspect = function() {
  ...
  if (
    this.is_tv ||
    (module.tv_archive && this.cur_media_item.mark_archive) ||
    this.active_time_shift ||
    this.active_local_time_shift
  ) {
    var ch_id =
      this.is_tv || this.active_time_shift || this.active_local_time_shift
        ? this.cur_media_item.id
        : this.cur_media_item.ch_id;
    ...
    if (this.ch_aspect[ch_id] !== undefined) {
      ...
      this.ch_aspect_idx = stb.aspect_array.getIdxByVal(
        "mode",
        this.ch_aspect[ch_id]
      );

      _debug("aspect_alias", stb.aspect_array[this.ch_aspect_idx].alias);
    } else {
      _debug(
        "this.profile[tv_channel_default_aspect]",
...
player.prototype.audio_pid = {
  ...
  set: function(pid) {
    ...
    if (stb.player.is_tv) {
      ...
      var idx = stb.player.channels.getIdxByVal(
        "id",
        stb.player.cur_media_item.id
      );
      ...
      idx = stb.player.fav_channels.getIdxByVal(
        "id",
        stb.player.cur_media_item.id
      );
      ...
      idx = module.tv.data_items.getIdxByVal(
        "id",
        stb.player.cur_media_item.id
      );
  ...
  get_for_menu: function() {
    ...
    for (var i = 0; i < this.all_pids.length; i++) {
      ...
      title =
        get_word("player_track") +
        " " +
        (i + 1) +
        lang +
        (this.all_pids[i].hasOwnProperty("type") &&
        stb.player.atrack_types.hasOwnProperty(this.all_pids[i].type)
          ? " [" + stb.player.atrack_types[this.all_pids[i].type] + "]"
          : "");
...
player.prototype.subtitle_pid = {
  ...
  get_all: function() {
    ...
    _debug(
      "stb.profile[always_enabled_subtitles]",
      stb.profile["always_enabled_subtitles"]
    );

    if (
      stb.profile["always_enabled_subtitles"] ||
      stb.player.cur_media_item.cmd.indexOf(" strack:") != -1
    ) {
      this.enable();
    } else {
      this.disable();
  ...
  get_for_menu: function() {
    ...
    for (var i = 0; i < this.all_pids.length; i++) {
      ...
      map.push({
        ...
        cmd: (function(pid) {
          return function() {
            stb.player.subtitle_pid.set(pid);
          };
        })(this.all_pids[i].pid),
        active: this.all_pids[i].selected
      });
...
player.prototype.init_con_menu = function() {
  ...
  var claim_cmd = [
    {
      cmd: function() {
        stb.player.send_claim("sound");
      },
      title: get_word("player_on_sound")
    },
    {
      cmd: function() {
        stb.player.send_claim("video");
      },
  ...
  if (this.is_tv) {
    claim_cmd = claim_cmd.concat([
      {
        cmd: function() {
          stb.player.send_claim("no_epg");
        },
        title: get_word("no epg")
      },
      {
        cmd: function() {
          stb.player.send_claim("wrong_epg");
        },
  ...
  var ClockMap = {
    ...
    cmd: [
      {
        cmd: function() {
          stb.player.ClockOnVideo.changeType("Off");
        },
      ...
      {
        cmd: function() {
          stb.player.ClockOnVideo.changeType("upLeft");
        },
      ...
      {
        cmd: function() {
          stb.player.ClockOnVideo.changeType("downRight");
        },
      ...
      {
        cmd: function() {
          stb.player.ClockOnVideo.changeType("downLeft");
        },
  ...
  var TempMap = {
    ...
    cmd: [
      {
        cmd: function() {
          stb.player.TempOnVideo.changeType("Off");
        },
        title: "Откл.",
        active: stb.player.TempOnVideo.position == "Off"
      },
      {
        cmd: function() {
          stb.player.TempOnVideo.changeType("upRight");
        ...
        title: "Сверху справа",
        active: stb.player.TempOnVideo.position == "upRight"
      },
      {
        cmd: function() {
          stb.player.TempOnVideo.changeType("upLeft");
        },
        title: "Сверху слева",
        active: stb.player.TempOnVideo.position == "upLeft"
      },
      {
        cmd: function() {
          stb.player.TempOnVideo.changeType("downRight");
        },
        title: "Снизу справа",
        active: stb.player.TempOnVideo.position == "downRight"
      },
      {
        cmd: function() {
          stb.player.TempOnVideo.changeType("downLeft");
        },
        title: "Снизу слева",
        active: stb.player.TempOnVideo.position == "downLeft"
      }
    ],
    type: "switch"
  };

  // console.log('map1= '+encodeURIComponent(JSON.stringify(map)));
...
player.prototype.build_con_menu = function() {
  ...
  if (this.con_menu.map.length > 5) {
    return;
  }

  if (stb && stb.Set3DConversionMode && stb.mac.substring(0, 8) != "10:27:BE") {
    this.con_menu.map.unshift({
      title: get_word("3D mode"),
      type: "switch",
      cmd: [
        {
          title: get_word("mode {0}").format(1),
          cmd: function() {
            _debug("mode 1");
            stb.Set3DConversionMode(0);
          },
        ...
        {
          title: get_word("mode {0}").format(2),
          cmd: function() {
            _debug("mode 2");
            stb.Set3DConversionMode(1);
          }
        },
        {
          title: get_word("mode {0}").format(3),
          cmd: function() {
            _debug("mode 3");
            stb.Set3DConversionMode(2);
          }
        },
        {
          title: get_word("mode {0}").format(4),
          cmd: function() {
            _debug("mode 4");
            stb.Set3DConversionMode(3);
          }
        }
      ]
    });
...
player.prototype.progress_bar = {
  ...
  stop: function() {
    ...
    var self = this;
    window.clearTimeout(this.hide_timeout);
    this.hide_timeout = window.setTimeout(function() {
      self.dom_obj.hide();
  ...
  init: function() {
    ...
    stb.clock.addCustomEventListener("tick", function(date) {
      if (self.on && stb.player.on) {
        self.osd_clock.innerHTML = get_word("time_format").format(
          date.hours,
          date.minutes,
          date.ap_hours,
          date.ap_mark
...
//END Clock On Video
//Temperature On Video by IgorK
player.prototype.TempOnVideo = {
  on: false,
  position: "Off",
  init: function() {
    _debug("player.init_TemperatureOnVideo");
    this.dom_obj = document.createElement("div");
    this.dom_obj.className = "osd_temperature_block";
    document.body.insertBefore(this.dom_obj, document.querySelector(".loader"));
    this.osd_temp = create_block_element("osd_temperature", this.dom_obj);
    var temp = stb.RDir("getenv temp_on_player");
    if (temp == "") {
      this.position = "Off";
    } else {
      this.position = temp;
    ...
    this.dom_obj.addClass("onVideoTemp" + this.position);
    this.osd_temp.innerHTML = "--.-";
  },
  hide: function() {
    this.dom_obj.hide();
  },
  update_temp: function(value) {
    if (typeof value != "undefined") {
      this.osd_temp.innerHTML = value;
    }
  },
  changeType: function(value) {
    this.dom_obj.removeClass("onVideoTemp" + this.position);
    this.position = value;
    setTimeout(
      "stb.RDir('setenv temp_on_player '+ player.prototype.TempOnVideo.position);",
      1000
    );
    this.dom_obj.addClass("onVideoTemp" + value);
    if (this.position == "Off") {
      this.on = false;
    } else {
      this.on = true;
