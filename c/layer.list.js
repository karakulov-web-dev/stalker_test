...
ListLayer.prototype.fill_list = function(data) {
  ...
  for (var i = 0; i < data.length; i++) {
    ...
    if (
      data[i].hasOwnProperty("open") &&
      (!data[i].open || data[i].only_for_moderator == 1)
    ) {
  ...
  if (
    this.result &&
    this.result.hasOwnProperty("selected_item") &&
    this.result.hasOwnProperty("cur_page") &&
    this.result.selected_item == 0 &&
    this.result.cur_page == 0
  ) {
    if (this.page_dir > 0) {
      this.cur_row = 0;
    } else {
...
ListLayer.prototype.set_active_row = function(num) {
  ...
  try {
    ...
    if (this.data_items[num]) {
      if (
        this.data_items[num].hasOwnProperty("open") &&
        (!this.data_items[num].open ||
          this.data_items[num].only_for_moderator == 1)
      ) {
