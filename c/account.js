    ...
    function AccountConstructor(){
        ...
        this.init = function(){
         ...
         //   this.tab['main']         = new Showable(create_block_element("main_container", this.container)).setParent(this).setDependencies(this.tab, ['payment', 'agreement', 'terms']);
            this.tab['main']         = new Showable(create_block_element("main_container", this.container)).setParent(this).setDependencies(this.tab, ['payment']);
            ...
            this.tab['payment']   = new Showable(create_block_element("payment_container", this.container)).setParent(this).setDependencies(this.tab, ['main']);
...
/*
     ...
     this.fill_main_info = function(result){
      ...
      var balance_rikt=0;
      var balance_ttk=0;
      var balance_beeline=0;
      var balance_zsttk=0;
      var show;
               
      var xmlhttp = new XMLHttpRequest();
      var url = "http://www.rikt.ru/service/get_balance_stb.php";
...
//	 console.log('responseText = '+xmlhttp.responseText);
    ...
    if (show=='true') {
     ls =  myArr[0].accountNumber; 
     billing_tariff =  myArr[0].billing_tariff;
     //balance  = myArr[0].balance.subtotal<0?'Задолженность <b>'+Math.abs(myArr[0].balance.subtotal)+'</b> руб.':'На Вашем счету <b>'+myArr[0].balance.subtotal+'</b> руб.';
	
	
	if (myArr[0].balance) {
   	  if (myArr[0].balance.subtotal) {
  	     balance  = myArr[0].balance.subtotal;
	   }
         }	
	
	if (myArr[0].balance_details) {
     if (myArr[0].balance_details.rikt) {
        if (myArr[0].balance_details.rikt.subtotal) balance_rikt  = myArr[0].balance_details.rikt.subtotal<0?'Задолженность <b>'+Math.abs(myArr[0].balance_details.rikt.subtotal)+'</b> руб.':'На Вашем счету <b>'+myArr[0].balance_details.rikt.subtotal+'</b> руб.';
      }  
        
     if (myArr[0].balance_details.ttk) {
       if (myArr[0].balance_details.ttk.subtotal) balance_ttk  = myArr[0].balance_details.ttk.subtotal<0?'Задолженность <b>'+Math.abs(myArr[0].balance_details.ttk.subtotal)+'</b> руб.':'На Вашем счету <b>'+myArr[0].balance_details.ttk.subtotal+'</b> руб.';
     }
     
     if (myArr[0].balance_details.beeline) {
      if (myArr[0].balance_details.beeline.subtotal) balance_beeline  = myArr[0].balance_details.beeline.subtotal<0?'Задолженность <b>'+Math.abs(myArr[0].balance_details.beeline.subtotal)+'</b> руб.':'На Вашем счету <b>'+myArr[0].balance_details.beeline.subtotal+'</b> руб.';
     }
     
     if (myArr[0].balance_details.zsttk) {
       if (myArr[0].balance_details.zsttk.subtotal) balance_zsttk  = myArr[0].balance_details.zsttk.subtotal<0?'Задолженность <b>'+Math.abs(myArr[0].balance_details.zsttk.subtotal)+'</b> руб.':'На Вашем счету <b>'+myArr[0].balance_details.zsttk.subtotal+'</b> руб.';
     }
	}
   }
      ...
      info += '<span class="label">' +get_word('MAC') + ':</span> ' + stb.mac + '<br>';
            ...
            if (result['tariff_plan']){
                info += '<span class="label">Ваш пакет КЛИК-ТВ: </span> ' + result['tariff_plan'] + '<br>';
            }

       if (show=='true') {  
          
           if (ls){
                info += '<span class="label">Лицевой счет: </span> ' + ls + '<br>';
            }
            
            if (billing_tariff){
                info += '<span class="label">Ваш тариф: </span> ' + billing_tariff + '<br><br>';
            }
            
            //if (balance){
               // info += '<span class="label">' +get_word('Account balance') + ':</span> ' + balance + '<br>';
            //}
            
           
            if (balance ==0 ) {
                 info += '<span class="label">Баланс по поставщикам услуг: 0 руб.</span>' + '<br>';
            }  else {
                 info += '<span class="label">Баланс по поставщикам услуг:</span>' + '<br>';
            }          
            
            if (balance_rikt){
                info += '<span class="label">РИКТ: </span> ' + balance_rikt + '<br>';
            }
            
            if (balance_ttk){
                info += '<span class="label">Компания Транстелеком: </span> ' + balance_ttk + '<br>';
            }
            
            if (balance_beeline){
                info += '<span class="label">ВЫМПЕЛ-КОММУНИКАЦИИ ОАО: </span> ' + balance_beeline + '<br>';
            }
            
            if (balance_zsttk){
                info += '<span class="label">Зап-СибТранстелеком: </span> ' + balance_zsttk + '<br>';
            }
         }   
            info += '<br>';
            info += '<span class="label">По вопросам оплаты Вы можете обратиться по следующим телефонам:</span>' + '<br>';
            info += '<b>6-50-00</b> – центр поддержки клиентов <br>';
//		info += '<b>6-50-59</b> – оператор по расчетам с населением <br>';
