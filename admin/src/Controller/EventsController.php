...
class EventsController extends \Controller\BaseStalkerController {
    ...
    public function __construct(Application $app) {
        ...
        $this->formEvent = array(
            ...
            array("id" => "diagnostic_page",    "title" => $this->setlocalization('Diagnostic Page') ),
            array("id" => "set_variable",       "title" => $this->setlocalization('Set Variable') ),
            array("id" => "set_volume",         "title" => $this->setlocalization('Set Volume') ),
   ...
   private function set_event_diagnostic_page(&$event, $user_list){
        $event->sendDiagnosticPage();
        return TRUE;
    }

    private function set_event_set_variable(&$event, $user_list){
        $event->sendSetVariable($this->postData['msg']);
        return TRUE;
    }

    private function set_event_set_volume(&$event, $user_list){
        $event->sendSetVolume($this->postData['channel']);
        return TRUE;
    }
